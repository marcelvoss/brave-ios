...
class FavoritesSectionProvider: NSObject, NTPObservableSectionProvider {
    ...
    func collectionView(_ collectionView: UICollectionView, contextMenuConfigurationForItemAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        ...
        return UIContextMenuConfiguration(identifier: indexPath as NSCopying, previewProvider: nil) { (elemenst) -> UIMenu? in
            let copy = UIAction(title: "Copy", image: UIImage(systemName: "doc.on.doc", withConfiguration: UIImage.SymbolConfiguration(scale: .medium))) { _ in
            }
            let openInNewTab = UIAction(title: "Open In New Tab", image: UIImage(systemName: "plus", withConfiguration: UIImage.SymbolConfiguration(scale: .medium)), identifier: nil, discoverabilityTitle: nil) { (action) in
                
            }
            let urlMenu = UIMenu(title: "", options: .displayInline, children: [copy, openInNewTab])
            let edit = UIAction(title: "Edit Favorite", image: UIImage(systemName: "pencil", withConfiguration: UIImage.SymbolConfiguration(scale: .medium)), identifier: nil, discoverabilityTitle: nil) { (action) in
                
            }
            let delete = UIAction(title: "Remove Favorite", image: UIImage(systemName: "trash", withConfiguration: UIImage.SymbolConfiguration(scale: .medium)), identifier: nil, discoverabilityTitle: nil, attributes: .destructive) { (action) in
