...
private class DuckDuckGoCalloutCell: NewTabCollectionViewCell<DuckDuckGoCalloutButton> {
    ...
    override func preferredLayoutAttributesFitting(_ layoutAttributes: UICollectionViewLayoutAttributes) -> UICollectionViewLayoutAttributes {
        ...
        attributes.size = view.systemLayoutSizeFitting(layoutAttributes.size, withHorizontalFittingPriority: .required, verticalFittingPriority: .fittingSizeLevel)
        return attributes
    }
}
...
class DuckDuckGoCalloutSectionProvider: NSObject, NTPObservableSectionProvider {
    ...
    private var isShowingCallout: Bool = false
    ...
    func updateCalloutVisibility() {
        ...
        isShowingCallout = isPrivateBrowsing && !isSearchEngineSet && shouldShowPromo
    }
    
    @objc private func privateModeChanged() {
        let oldValue = isShowingCallout
        updateCalloutVisibility()
        if oldValue != isShowingCallout {
            sectionDidChange?()
        }
    }
